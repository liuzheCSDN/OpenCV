...
int main()
{
	...
	input = imread("G:\\opencv_test\\easyPR_soble\\easyPR_soble\\16.jpg");
	...
	for (int i = 0; i < rects.size(); i++)
	{
		...
		if (a)
		{
			...
			getHistogramFeatures(Result_threshold,feature);

			//分类车牌号与非车牌号
			CvSVM svm;
			string path = "G:/opencv_test/easyPR_soble/easyPR_soble/svm.xml";
			svm.load(path.c_str(),"svm");

			//Mat img = Mat(pos,true);
			//Mat p = img.reshape(1, 1);
			//p.convertTo(p, CV_32FC1);
			//int pred =(int)svm.predict(p); //返回标签
			int class1= (int)svm.predict(feature);
			//printf("是否为车牌%d\n", pred);
			printf("是否为车牌%d\n", class1);
		}
